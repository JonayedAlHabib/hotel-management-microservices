...
export default function RoomsPage() {
  ...
  return (
    <div className="w-full px-6 py-6 space-y-5">
      <div>
        <h1 className="text-xl font-semibold text-navy-900">Rooms</h1>
        ...
        {roomTypes?.map((room) => {
          ...
          return (
            <Link
              key={room.id}
              to={`/rooms/${room.id}`}
              className="bg-white rounded-2xl border border-navy-100 overflow-hidden hover:shadow-md transition-shadow block"
            >
              <div className="h-36 bg-navy-50 flex items-center justify-center overflow-hidden">
                {photo ? (
              ...
              <div className="p-4 space-y-1">
                ...
                {room.description && <p className="text-xs text-navy-400 line-clamp-2">{room.description}</p>}
